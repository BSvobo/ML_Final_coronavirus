...
for _dist in ['minkowski']:
    for val in np.unique(confirmed["Country/Region"]):
        # test data
        df = data.filter_by_attribute(
            confirmed, "Country/Region", val)
        cases, labels = data.get_cases_chronologically(df)
        #print("Cases at the top", cases)

        if cases[0][-1] > MIN_CASES and labels[0,1] != "Turkey" and labels[0,1] != "US" and labels[0,1] != "India" and labels[0,1] != "Spain" and labels[0,1] != "Brazil":# or labels[0,1] == "Tunisia" or labels[0,1] == "Slovakia":
            NUM_COLORS += 1
        ...
        #print("Cases after calling np.diff:", cases)
    ...
    for val in np.unique(confirmed["Country/Region"]):
        df = data.filter_by_attribute(
            confirmed, "Country/Region", val)
        cases, labels = data.get_cases_chronologically(df)
        cases_diff = np.diff(cases.sum(axis=0, keepdims=True), axis=-1)
        print("CASES DIFF", cases_diff)

        if cases[0][-1] > MIN_CASES and labels[0,1] != "Turkey" and labels[0,1] != "US" and labels[0,1] != "India" and labels[0,1] != "Spain" and labels[0,1] != "Brazil":# or labels[0,1] == "Tunisia" or labels[0,1] == "Slovakia":
            i = len(legend)
            lines = ax.plot(cases_diff[0], label=labels[0,1])
            handles.append(lines[0])
            lines[0].set_linestyle(LINE_STYLES[i%NUM_STYLES])
            lines[0].set_color(colors[i])
            legend.append(labels[0, 1])

    ax.set_ylabel('# of new daily deaths')
    ax.set_xlabel("Time (days since Jan 22, 2020)")

    ax.set_yscale('linear')
    ax.legend(handles, legend, bbox_to_anchor=(0., 1.02, 1., .102), loc=3, ncol=4)
    plt.tight_layout()
    plt.savefig('results/daily_deaths_by_country.png')
